...
plt.title('Surface Air Temperature (T)')
...
if line_1_99_top:
    ...
    plt.plot([range_1_99[0],range_1_99[0]],[7,-3],color='0.5', lw=0.6, clip_on=False)
    plt.plot([range_1_99[1],range_1_99[1]],[7,-3],color='0.5', lw=0.6, clip_on=False)
    
    plt.plot([range_1_99[0],range_1_99[1]],[-7./8.,-7./8],color='0.5', lw=0.6, clip_on=False)
    plt.text( (range_1_99[0]+range_1_99[1])/2.0,-7./8.,'98%',va='center',ha='center',
...
if frac_100:
    plt.ylabel('Full-SG anomaly ($^\circ$C)')
else:
    plt.ylabel('Half-SG anomaly ($^\circ$C)')
...
if line_1_99_top:
    ...
    plt.plot([range_1_99[0],range_1_99[0]],[2,-3],color='0.5', lw=0.6, clip_on=False)
    plt.plot([range_1_99[1],range_1_99[1]],[2,-3],color='0.5', lw=0.6, clip_on=False)
    
    plt.plot([range_1_99[0],range_1_99[1]],[-2.5,-2.5],color='0.5', lw=0.6, clip_on=False)
    plt.text( (range_1_99[0]+range_1_99[1])/2.0,-2.5,'98%',va='center',ha='center',
             clip_on=False, color='black', bbox=dict(facecolor='white', edgecolor='white'))

if angled_labels:
    px_tp_pe = 4./7. # reproduce same proportions as for PE using this factor
...
plt.xlabel('2xCO$_{2}$ anomaly ($^\circ$C)')
...
rect = patches.Rectangle((2.2,18),2.,82,linewidth=1,edgecolor='k',facecolor='white',clip_on=False)

# Add the patch to the Axes
ax_p5_mid.add_patch(rect)

plt.text(2.3, 83, "Moderated,\nsignificant",clip_on=False, color=blue, 
         va="baseline", ha="left", multialignment="left")
plt.text(2.3, 63, "Moderated,\ninsignificant",clip_on=False, color=l_blue,
         va="baseline", ha="left", multialignment="left")
plt.text(2.3, 43, "Exacerbated,\ninsignificant",clip_on=False, color=l_red,
         va="baseline", ha="left", multialignment="left")
plt.text(2.3, 23, "Exacerbated,\nsignificant",clip_on=False, color=red,
         va="baseline", ha="left", multialignment="left")
...
ax_pe_mid.set_aspect(7./100.) # set so bottom panel has same aspect ratio as top
ax_p5_mid.set_aspect(4./100.)
...
plt.savefig(out_dir+'fig_2_t_p.png', format='png', dpi=480)
plt.savefig(out_dir+'fig_2_t_p.eps', format='eps', dpi=480)
